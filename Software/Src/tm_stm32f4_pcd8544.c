...
void
PCD8544_InitIO(SPI_HandleTypeDef *spi) {
...
void
PCD8544_send(unsigned char data) {
    PCD8544_CE_LOW;
    HAL_SPI_Transmit(PCD8544_SPI, &data, 1, 100);
    PCD8544_CE_HIGH;
}

void
PCD8544_Pin(PCD8544_Pin_t pin, PCD8544_State_t state) {
    switch (pin) {
        case PCD8544_Pin_DC:
...
void
PCD8544_Delay(unsigned long micros) {
...
void
PCD8544_Init(SPI_HandleTypeDef *spi, unsigned char contrast) {
    //Initialize IO's
    PCD8544_InitIO(spi);
    //Reset
...
void
PCD8544_Write(PCD8544_WriteType_t cd, unsigned char data) {
    switch (cd) {
...
void
PCD8544_SetContrast(unsigned char contrast) {
    // Go in extended mode
    PCD8544_Write(PCD8544_COMMAND, PCD8544_FUNCTIONSET | PCD8544_EXTENDEDINSTRUCTION);
...
void
PCD8544_DrawPixel(unsigned char x, unsigned char y, PCD8544_Pixel_t pixel) {
    if (x >= PCD8544_WIDTH) {
        return;
    }
    if (y >= PCD8544_HEIGHT) {
...
void
PCD8544_Invert(PCD8544_Invert_t invert) {
...
void
PCD8544_UpdateArea(unsigned char xMin, unsigned char yMin, unsigned char xMax, unsigned char yMax) {
...
void
PCD8544_GotoXY(unsigned char x, unsigned char y) {
    PCD8544_x = x;
    PCD8544_y = y;
}

void
PCD8544_Putc(char c, PCD8544_Pixel_t color, PCD8544_FontSize_t size) {
    unsigned char c_height, c_width, i, b = 0, j;
...
void
PCD8544_Puts(char* c, PCD8544_Pixel_t color, PCD8544_FontSize_t size) {
    while (*c) {
...
void
PCD8544_DrawLine(unsigned char x0, unsigned char y0, unsigned char x1, unsigned char y1, PCD8544_Pixel_t color) {
    short dx, dy;
    short temp;
...
void
PCD8544_DrawRectangle(unsigned char x0, unsigned char y0, unsigned char x1, unsigned char y1, PCD8544_Pixel_t color) {
...
void
PCD8544_DrawFilledRectangle(unsigned char x0, unsigned char y0, unsigned char x1, unsigned char y1, PCD8544_Pixel_t color) {
    for (; y0 < y1; y0++) {
...
void
PCD8544_DrawCircle(char x0, char y0, char r, PCD8544_Pixel_t color) {
    int16_t f = 1 - r;
    int16_t ddF_x = 1;
    int16_t ddF_y = -2 * r;
    int16_t x = 0;
...
void
PCD8544_DrawFilledCircle(char x0, char y0, char r, PCD8544_Pixel_t color) {
    int16_t f = 1 - r;
    int16_t ddF_x = 1;
    int16_t ddF_y = -2 * r;
    int16_t x = 0;
